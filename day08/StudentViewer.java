...
//						ScannerUtil.nextInt(scanner, "국어점수: ", 0, 100),
//						ScannerUtil.nextInt(scanner, "영어 점수: ", 0, 100),
//						ScannerUtil.nextInt(scanner,"수학점수: ",0, 100)));
		
		
	}
	    private void selectAll() {
	    	printOne();
	    	String message = "상세보기 할 학생의 번호를 입력해주세요(0은 뒤로가지)\n >";
	...
	private void printList(int index) {
		...
		System.out.printf("총점: %03d점 평균: %.2f점\n",s.caluclateSum(),s.calculateAverage());
		String message =" 1. 수정  2. 삭제 3. 뒤로가기 \n >";
	}
	
	private void updatestudent(int index) {
		
		studentArray[index].setKorean(ScannerUtil.nextInt(scanner,"국어점수: ", 0, 100));
		studentArray[index].setEnglish(ScannerUtil.nextInt(scanner, "영어 점수: ", 0, 100));
		studentArray[index].setMath(ScannerUtil.nextInt(scanner, "수학점수: ",0, 100));
	
	
	}
	
	private void deletetudent(int index) {
