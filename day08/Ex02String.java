...
public class Ex02String {
	public static void main(String[] args) {
		...
		//    String에서 위치를 셀때는
		//    0부터 센다!
		System.out.println("str.charAt('a'): "+str.charAt(4));
		...
		// 6. 만약 String이 공백으로만 이루어졌는지 확인할 때는
		//    isBlank()을 실행하면 된다.
		str="  ";
...
//		System.out.println("str.isBlank(): "+str.isBlank());
		...
		// 7. 만약 String이 빈 문자열, 즉 아무런 문자도 없는지 확인할때에는
		//    isEmpty()을 실행하면 된다.
		str = new String();
		...
		// 8. 만약 문자열에서 파라미터가 가장 마지막으로 나온 위치가 알고 싶을 때는
		//    lastIndexOf()을 실행하면 된다.
		...
		//     패턴이 맞는지를 체크할 때는 mathches()를 실행하면 된다.
		//     정규표현식이란? = 숙제
		...
		System.out.println("str.matches(pattern): "+str.matches(pattern));
		pattern = "[가-힝]*";  //[가-힝]*은 문자열이 가~힝까지의 글자로 이루어져있는지를 확인함
		System.out.println("str.matches(pattern): "+str.matches(pattern));
		
		// 11. 해당 문자열이 특정 문자열로 시작하는지 확인할때는
		//     startsWith()로 확인한다.
		System.out.println("str.startsWith(\"1234\"): "+str.startsWith("1234"));
		System.out.println("str.startsWith(\"abcd\"): "+str.startsWith("abcd"));
		
		// 12. 문자열을 쪼갤때는 subString()을 사용한다.
		//     ****주의**** 쪼갠다고 해서 원본이 바뀌지는 않음
		
		// 파라미터가 int 1개이면 해당 인덱스부터 문자열이 끝날때까찌를 분리함
		...
		// 13. 문자열의 모든 글자를 소문자로 변환할 때는 toLowerCase()을 사용한다.
		//     ****주의**** 원본이 바뀌지는 않음
		str="abcDEFghi";
		System.out.println("str.toLowerCase(); "+str.toLowerCase());
		
		// 14. 문자열의 모든 글자를 대문자로 변활할 때는 toUpperCase()을 사용한다.
		System.out.println("str.toUpperCase(): "+str.toUpperCase());
		
		// 15. 문자열에 앞뒤 공백을 제거할때는 trim()을 사용한다.
		//     ***주의*** 원본이 바뀌지는 않음
		str = "              1         \n";
