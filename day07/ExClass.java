 ...
 * 1. 클래스
 *    클래스는 하나의 프로그램이다
 *    좀 더 자세히 말하면 하나의 프로그램 설계도 이다.
 *    객체지향 프로그래밍, 3세대 언어에서는
 ...
 *    클래스라는 것은 자체가 한가의 책임에 따라 거기에 알맞은 정보를 넣는 칸과, 그리고 기능들이 구현되어
 ...
 *    
 * 5. 메소드
 *    메소드란 해당 클래스의 모든 객체들이 고통적으로 가지고 있는 기능을
 *    메소드라고 한다.
 *    필드와는 다르게 모든 메소드들은 똑같은 로직으로 돌아간다.
 *    예를 들어, 학생의 국영수 점수를 합쳐서 총점을 게산하는 메소드는
 *    A 학생이건, B 학생이건 똑같다.
 *    
 * 6. 생성자 (Constructor)
 ...
 *    해당 클래스의 필드를 초기화해준다.
 *    만약 해당 필드의 데이터타입이 창조형이면
 *    null로 초기화하고
 *    기본형 데이터타입이면 0으로 초기화한다!
 *    
 *    만약 사용자가 생성자를 따로 만들어준다면
 *    기본생성자가 호출돼서 
 *    클래스의 해당 필드를 null과 0으로 초기화 하고         
 ...
 *    따라서, 파라미터 있는 생성자와 파라미터 없는 생서앚 모두 다 필요하다면
 *    둘다 만들어야 한다!
 *    만약 해당 객체가 외부로부터 어떠한 값을 반드시 받아와야 한다면
 *    파라미터있는 생상자만 만들어서 외부의 값을 강제 할 수도 있다!
 *    
 * 7. 필드와 파라미터의 호출 우선도
 ...
 *    자바가 기본적으로 제공하던 아니면 누군가가 만들었던 자바의  존재하는 모든 클래스는
 *    java.lang.Object를 상속 받는다.
 *    상속이란, 부모 클래스이 메소드, 필드를 자식 클래스가 다시 안써줘도 그대로 받아서 쓸 수 있게
 *    "코드를 물려주는" 것이다.
 *    java.lang.Object는 내부적으로
 *    기본 생성자, toString(), equals() 등의 다양한 메소드를 선언과 구현해서 자식 클래스가
 *    또 다시 선언과 정의를 하지 않아도 쓸 수 있께 만들어놓았다.
 ...
 *       반드시 equals를 override 해야한다.
 *       java.lang.object의 equlas 메소드는 다음과 같이 되어있다.
 *       
