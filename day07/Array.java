...
public class Array {
    ...
    // Student 구조체에 대한 
    // 1. contains
    public static boolean contains(Ard[] array, Ard s) {
        for(int i = 0; i < array.length; i++) {
            if(array[i].id == s.id && array[i].name.equals(s.name)) {
                return true;
            }
        }
    ...
    // 2. indexOf
    public static int indexOf(Ard[] array, Ard s) {
        for(int i = 0; i < array.length; i++) {
            if(array[i].id == s.id && array[i].name.equals(s.name)) {
                return i;
            }
    ...
    // 3. add
    public static Ard[] add(Ard[] array, Ard s) {
    	Ard[] temp = new Ard[array.length];
        for(int i = 0; i < array.length; i++) {
            temp[i] = array[i];
        }
        array = new Ard[temp.length + 1];
        for(int i = 0; i < temp.length; i++) {
            array[i] = temp[i];
        }
        array[temp.length]= s;
    ...
    // 4. removeByIndex
    public static Ard[] removeByIndex(Ard[] array, int index) {
        if(index < 0 || index > array.length - 1) {
            return array;
        }
    ...
    // 5. remove 를 작성하시오.
    public static Ard[] remove(Ard[] array, Ard s) {
        return removeByIndex(array, indexOf(array, s));
    }
    // 6. size 를 작성하시오
    public static int size(Ard[] array) {
        return array.length;
    }
